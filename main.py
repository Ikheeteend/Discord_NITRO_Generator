    ...
    def main(self):  # The main function contains the most important code
        ...
        if os.name == "nt":  # If the system is windows
            ...
            ctypes.windll.kernel32.SetConsoleTitleW(
                "Nitro Generator and Checker - Made by JB™#7184")  # Change the
        else:  # Or if it is unix
            print(f'\33]0;Nitro Generator and Checker - Made by Gonku#0001\a',
                  end='', flush=True)  # Update title of command prompt

        print("""""")  # Print the title card
        time.sleep(2)  # Wait a few seconds
        # Print who developed the code
        self.slowType("Made by: JB™#7184", .02)
        ...
        if USE_WEBHOOK:
            ...
            if webhook is not None:
                DiscordWebhook(  # Let the user know it has started logging the ids
                        url=url,
                        content=f"```Started checking urls\nI send working discord nitro gifts here```"
                    ).execute()
        ...
        for s in c:  # Loop over the amount of codes to check
            ...
            if os.name == "nt":  # If the system is windows
                ctypes.windll.kernel32.SetConsoleTitleW(
                    f"Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by JB™#7184")  # Change the title
                print("")
            else:  # If it is a unix system
                # Change the title
                print(
                    f'\33]0;Nitro Generator and Checker - {len(valid)} Valid | {invalid} Invalid - Made by JB™#7184\a', end='', flush=True)
